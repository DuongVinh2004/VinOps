...
export async function verifyMigrations(connectionString) {
    const pool = new Pool({ connectionString, application_name: 'vinops-migration-verify', max: 1 });
    try {
        const expected = await migrations();
        const appliedResult = await pool.query('SELECT name, checksum FROM vinops.schema_migrations ORDER BY name');
        const applied = appliedResult.rows;
        if (applied.length !== expected.length) {
            throw new Error(`Migration verification expected ${expected.length} records, found ${applied.length}.`);
        }
        ...
        const rlsResult = await pool.query(`SELECT count(*)::text AS protected_tables
         FROM pg_tables
        WHERE schemaname = 'vinops' AND rowsecurity`);
        if (Number(rlsResult.rows[0]?.protected_tables ?? '0') < 28) {
...
export async function runMigrations(connectionString) {
    ...
    try {
        ...
        await client.query("SELECT pg_advisory_xact_lock(hashtext('vinops:platform-foundation:migration'))");
    ...
    `);
        const applied = await client.query('SELECT name, checksum FROM vinops.schema_migrations ORDER BY name FOR UPDATE');
