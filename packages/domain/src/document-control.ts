...
const revisionTransitions: Record<
  RevisionStatus,
  Partial<Record<RevisionAction, RevisionStatus>>
> = {
...
export function revisionTransitionTarget(
  ...
  if (target === undefined) {
    throw new DomainError(
      ...
      `Revision action ${action} is not permitted from ${current}.`,
    );
  }
  return target;
}
...
export function assertWithdrawAllowed(input: {
  ...
  if (input.isCurrent) {
    throw new DomainError(
      ...
      'A current revision cannot be withdrawn.',
    );
  }
...
export function evaluateReviewRoute(
  policy: ReviewRoutePolicy,
  assignments: readonly ReviewAssignmentSnapshot[],
): ReviewRouteResult {
  ...
  if (
    ...
    throw new DomainError(
      ...
      'Review quorum and reject threshold are invalid.',
    );
  }
  ...
  if (
    assignments.some(
      (assignment) => !Number.isInteger(assignment.sequence) || assignment.sequence < 1,
    )
  ) {
    throw new DomainError(
      'REVIEW_SEQUENCE_INVALID',
      'Reviewer sequence must be a positive integer.',
    );
  }
...
export function assertReviewerMayDecide(input: {
  ...
  if (input.actorUserId === input.makerUserId) {
    throw new DomainError(
      ...
      'The revision maker cannot decide its review.',
    );
  }
  ...
  if (input.existingDecision) {
    throw new DomainError(
      ...
      'A review decision cannot be edited in place.',
    );
  }
}
...
export function assertPublishAllowed(input: {
  ...
  if (input.status === 'Approved with Comments' && input.unresolvedMandatoryComments > 0) {
    throw new DomainError(
      ...
      'All mandatory review comments require a disposition before publication.',
    );
  }
...
export function normalizeDocumentCode(value: string): string {
  ...
  if (code.length < 1 || code.length > 120 || !/^[A-Z0-9][A-Z0-9._/-]*$/u.test(code)) {
    throw new DomainError(
      ...
      'Document code does not satisfy the project policy.',
    );
  }
  return code;
}
...
export function assertDeclaredFilePolicy(
  policy: FilePolicy,
  input: { sizeBytes: number; mediaType: string; filename: string; sha256: string },
): void {
...
const magicChecks: Readonly<Record<string, (bytes: Uint8Array) => boolean>> = {
...
export function assertMagicBytes(mediaType: string, bytes: Uint8Array): void {
...
export function assertAnnotation(input: {
  ...
  if (
    ...
    throw new DomainError(
      'ANNOTATION_COORDINATES_INVALID',
      'Annotation coordinates must be normalized to 0-1.',
    );
  }
...
export function signedUrlExpiry(now: Date, ttlSeconds: number): Date {
  if (!Number.isInteger(ttlSeconds) || ttlSeconds < 15 || ttlSeconds > 300) {
    throw new DomainError(
      'SIGNED_URL_TTL_INVALID',
      'Signed URL TTL must be between 15 and 300 seconds.',
    );
  }
